...
CSV_TEXT.split('\n').each { fullName ->
    ...
    try {
        def q = QueryPool.query("name = \"$fullName\"").setMaxResults(1).execute()
        if (q?.list()?.size() > 0) emp = q.list()[0]
    } catch (Exception e) {}
    
    if (!emp) {
        try {
            def q = QueryPool.query("name LIKE \"%$fullName%\"").setMaxResults(1).execute()
            if (q?.list()?.size() > 0) emp = q.list()[0]
        } catch (Exception e) {}
    }
...
${stats.withLicenseList.isEmpty() ? '' : stats.withLicenseList.collect { "  - $it" }.join('\n')}

БЕЗ ЛИЦЕНЗИИ: ${stats.withoutLicense}
${stats.withoutLicenseList.isEmpty() ? '' : stats.withoutLicenseList.collect { "  - $it" }.join('\n')}
"""
